...
import { SingleCOllectionType } from "@/@types/SingleCollectionType"

const SingleCollectionCard:FC<{item:SingleCOllectionType}> = ({item}) => {
  return (
      ...
      <div className="flex justify-between mt-[15px]">
        ...
        <div className="mt-[15px]">
            <Heading classList="!mb-[10px]" tag="h3">{item.title}</Heading>
              <Text classList="!font-normal">{item.title}</Text>
        </div>
    </div>
  )
}
